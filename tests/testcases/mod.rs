...
pub mod all {
    ...
    pub async fn http_go_works(controller: &dyn Controller) {
        ...
        // expected trait `for<'a> <for<'a> fn(&'a AppMetadata, Option<tokio::io::BufReader<tokio::process::ChildStdout>>) -> impl Future<Output = Result<(), anyhow::Error>> {checks} as FnOnce<(&'a AppMetadata, Option<tokio::io::BufReader<tokio::process::ChildStdout>>)>>`
        // found    trait `for<'a> <for<'a> fn(&'a AppMetadata, Option<tokio::io::BufReader<tokio::process::ChildStdout>>) -> impl Future<Output = Result<(), anyhow::Error>> {checks} as FnOnce<(&'a AppMetadata, Option<tokio::io::BufReader<tokio::process::ChildStdout>>)>>`
        ...
        TestCase::new_with_x(checks).run(controller).await.unwrap();
